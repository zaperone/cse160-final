...
df <- read.csv("~/Desktop/DataAnalyst.csv")
```
...
#Logistic regression
```{r}
clean_df$IsSalary <- ifelse(clean_df$Max_Salary > "80", 1, 0)
clean_df$IsRating <- ifelse(clean_df$Rating > "3.5", 1, 0)
clean_df$IsFounded <- ifelse(clean_df$Founded < "1995", 1, 0)
clean_df$IsSector <- ifelse(clean_df$Sector == "Business Services" , 1, 0)
clean_df$IsPrivate <- ifelse(clean_df$Type.of.ownership == "Company - Private" , 1, 0)
clean_df$IsPublic <- ifelse(clean_df$Type.of.ownership == "Company - Public" , 1, 0)
clean_df$IsNY <- ifelse(clean_df$Location == "New York, NY" , 1, 0)

g1 <- glm(IsSalary ~ IsRating  + IsFounded + IsSector + IsPrivate + IsPublic + IsNY, data = clean_df,
...
split_counts
#known errors: City of Industry, CA; DC Ranch, AZ; Far Rockaway, NY

#data courtesy of https://simplemaps.com/data/us-cities
us_cities_data <- read.csv("~/Desktop/uscities.csv")
head(us_cities_data)
...
f <- function(x, output) {
  ...
  if(dim(selection)[1] != 0) {
    #print("success!")
    print(selection$lat)#x[4] <- selection$lat
    #x[5] <- selection$lng
  }
  else {
    sprintf("INVALID CITY ERROR, CITY NAME: %s, %s", city, state)
  } #33 errors - look these up manually, or ignore
}
apply(split_counts, 1, f)
split_counts
```